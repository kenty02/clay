...
import {ensureId, searchHistoryByUrl} from "../utils";
...
const useHistory = (url: string) => {
  const [history, setHistory] = useState<browser.History.HistoryItem[]>([]);
  useEffect(() => {
    if (url !== "") searchHistoryByUrl(url).then(setHistory);
  }, [url]);
  return history;
};
...
const Cell: React.VFC<GridChildComponentProps> = ({
  ...
  const histories = useHistory(node?.url ?? "");
